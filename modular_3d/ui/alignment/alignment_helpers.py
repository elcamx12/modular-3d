...
def iter_component_rects(comp, layer):
    ...
    if isinstance(comp, Module):
        ...
        # [2026-05-30 C3] 수직 3층 모듈 — 일반 모듈처럼 통기둥·슬래브·보를 평면 표현.
        for col in comp.columns:
            yield column_xy(col), 'column'
        if layer == LAYER_BOTTOM:
            for slab in getattr(comp, 'slabs', []) or []:
                yield slab_xy(slab), 'slab'
            for grp in comp.bottom_beams:
                for beam in grp:
                    yield beam_xy(beam), 'beam'
        else:
            for beam in comp.top_beams:
                yield beam_xy(beam), 'beam'
