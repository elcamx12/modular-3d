...
from modular_3d.ui.controls.controls_f5 import F5Mixin
from modular_3d.ui.controls.controls_f6 import F6Mixin
...
class Controller(F5Mixin, F6Mixin):
    ...
    def __init__(self, viewer: Viewer3D, dim_panel: DimensionInputPanel,
        ...
        if analysis_panel is not None:
            ...
            # [2026-05-30] 컨투어 종류·자유도 색 토글 제거 — 관련 연결 삭제.
            # 물량산출 탭 응력비 시각화 (2026-05-08) — 유지.
    ...
    def _do_undo(self):
        ...
        if action.action_type == 'place':
            ...
            # 각각 복원하고 영향받은 모든 부재 메시를 한 번씩 재생성.
            inner = action.data.get('_inner_actions', [])
            affected = set()
            for ia in reversed(inner):
                cid = ia.data['comp_id']
                idx = ia.data['index']
                comp = self._scene.components.get(cid)
                if comp is None:
                ...
                if ops is None:
                    continue
                if ia.action_type == 'opening_add':
                    if 0 <= idx < len(ops):
                        ops.pop(idx)
                elif ia.action_type == 'opening_del':
                    ops.insert(min(idx, len(ops)), dict(ia.data['op']))
                else:  # opening_move
                    if 0 <= idx < len(ops):
                        ops[idx] = dict(ia.data['old_op'])
                affected.add(cid)
            for cid in affected:
                comp = self._scene.components.get(cid)
                if comp is None:
            ...
            inner = action.data.get('_inner_actions', [])
            for ia in reversed(inner):
                if ia.action_type == 'room_add':
                    rid = ia.data['room_id']
                    self._scene.rooms.pop(rid, None)
                    if hasattr(self._viewer, 'remove_room_visual'):
                        self._viewer.remove_room_visual(rid)
                elif ia.action_type == 'room_del':
                    room = ia.data['room']
                    self._scene.rooms[room.id] = room
                    if hasattr(self, '_render_room_3d'):
                        self._render_room_3d(room)
                elif ia.action_type == 'room_move':
                    rid = ia.data['room_id']
                    room = self._scene.rooms.get(rid)
                    if room is not None:
                        room.polygon = [(float(x), float(y))
                                        for (x, y) in ia.data['old_polygon']]
                        if hasattr(self, '_render_room_3d'):
                            self._render_room_3d(room)
