...
`load_all_trucks` 는 두 계층을 머지해 반환한다. `save_project_truck` 은
project 계층에만 저장한다 (내장은 항상 read-only).

[도로 카탈로그 폐지 — 2026-05-26]
명명된 도로 등급(광로/일반/이면)을 폐지하고 프로젝트 설정의 현장 운송
제한(SiteLimit)으로 대체. 도로 로더·데이터(road_limits.json)는 제거됨.
...
- JSON 형식 → 데이터클래스 변환에서 ValueError 발생 시 어느 파일의 어느
  항목인지 메시지에 포함해 추적 가능.
- validate_truck 은 dict 인풋(예: UI 폼) 을 사전 검증하는 편의 함수.
"""
