...
def pack_items(
    ...
    """모듈·패널 → 회차 산정 (현장 제한 site + 트럭 한도로 검사).
    ...
    mod_trips, mod_blocked = _pack_modules(
        modules, trucks, site, spacing, start_trip_no=next_no,
    )
    ...
    dep_trips, dep_blocked, remaining = _pack_dependent_panels(
        dependent_panels, trucks, site, spacing, next_no,
    ...
    fl_trips, fl_blocked = _pack_floor_panels(
        rem_floor, trucks, site, spacing, next_no,
    )
    ...
    wl_trips, wl_blocked = _pack_wall_panels(
        rem_wall, trucks, site, spacing, next_no,
    )
